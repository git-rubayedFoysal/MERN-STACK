/* This JavaScript code snippet defines an array of products with various properties like id, name,
category, price, and inStock status. It then extends the array prototype with a custom filter
function called `cusFilter`. This custom filter function takes a callback function as an argument
and iterates over each object in the array, pushing the objects that pass the condition specified in
the callback function into a new array. */
...
products.cusFilter = function(compare) {
    const newArr = [];
    for (let obj of this) {
        if (compare(obj)) {
            newArr.push(obj);
    ...
    return newArr;
                     ...
                     console.log(products.cusFilter((product) => product.price >= 100 && product.inStock));
