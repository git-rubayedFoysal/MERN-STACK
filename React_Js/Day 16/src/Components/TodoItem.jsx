...
function TodoItem({ todo }) {
  ...
  const editTodo = () => {
    updateTodo({ ...todo, todo_msg: todoMsg }, todo.id);
    setIsTodoEditable(false);
  };

  return (
    <div
      className={`flex border border-black/10 rounded-lg px-3 py-1.5 gap-x-3 shadow-sm shadow-white/50 duration-300 font-bold  text-black ${
        todo.completed ? "bg-[#c6e9a7]" : "bg-white/50"
      }`}
    >
      <input
        type="checkbox"
